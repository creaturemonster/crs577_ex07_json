...
function displayInventory(resp){
	// resp.item is an array of Item objects automatically converted from JSON response
	var inventoryTable = $('#inventory');
	var len = resp.item.length;
	var html="<tr><th>Product Id</th><th>Quantity</th><th></th></tr>";
	for(var i=0; i < len; i++){
		var item = eval(resp.item[i]);
		brokenExHint(item);
